...
import {
    setNewAlertsFlag, getNewAlertsFlag,
    updateNotificationBellUI, showAlert,
    setNewWithdrawalRequestsFlag, getUserIdFromToken
} from './utils.js';
        ...
        ws.onmessage = (event) => {
            ...
            if (isServidorDashboard) {
                ...
                if (message.type === "withdrawal_status_update") {
                    ...
                    showAlert(`Sua solicitação de retirada ID ${message.retirada_id} foi atualizada para: ${message.status}.`, "primary", 5000);
                    try {
                        ...
                        console.error("Não foi possível criar objeto de áudio para notificação de retirada:", e);
                    }
                }
