            ...
            <div class="card-body">
                <form id="search-bar" class="row g-3 mb-0">
                    ...
                    <div class="col-12 col-md">
                        ...
                        <input type="text" id="search-categoria" class="form-control" placeholder="Buscar por categoria" value="${searchCategoria}">
                    </div>
...
// MODIFICADO: Adicionado parâmetro isReadOnly
...
async function renderizarListItens(isReadOnly = false) {
    try {
        ...
        if (main) {
            ...
            if (!isReadOnly) { // Only bind row actions if not in read-only mode
        ...
        btn.onclick = async () => {
            ...
            try {
                ...
                if (err.message.includes('retirada_item')) {
                    alert('Item vinculado a uma ou mais retiradas.\n\nPrimeiro exclua as retiradas relacionadas a este item.');
                } else {
                    alert(err.message);
                }
            }
