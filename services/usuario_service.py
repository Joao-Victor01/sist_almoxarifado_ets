...
class UsuarioService:
    ...
    async def create_usuario(db: AsyncSession, usuario_data: UsuarioCreate):
        existing_user = await db.scalar(select(Usuario).where(Usuario.username == usuario_data.username))
        if existing_user:
            raise HTTPException(
        ...
        existing_email = await db.scalar(select(Usuario).where(Usuario.email == usuario_data.email))
        if existing_email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
    ...
    async def delete_usuario(db: AsyncSession, usuario_id: int):
        ...
        has_dependencias = await db.execute(select(Usuario).filter(Usuario.id == usuario_id))
    ...
    async def update_usuario(db: AsyncSession, usuario_id: int, usuario_data: UsuarioUpdate):
        ...
        if usuario_data.email_usuario:
            ...
            existing_email = await db.scalar(select(Usuario).where(Usuario.email == usuario_data.email_usuario, Usuario.id != usuario_id))
            if existing_email:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, 
